...
class AcousticImpedance(AuxQuantity1Phase):
    ...
    def compute(self, data, der):
        ...
        drhoc_daA1 = der[self.rho]["aA1"] * data[self.c] + data[self.rho] * der[self.c]["aA1"]
        drhoc_darhoA = der[self.rho][self.arhoA] * data[self.c] + data[self.rho] * der[self.c][self.
                                                                                               arhoA]
        drhoc_darhouA = data[self.rho] * der[self.c][self.arhouA]
        drhoc_darhoEA = data[self.rho] * der[self.c][self.arhoEA]

        der[self.name]["aA1"] = drhoc_daA1
