...
class EnergyFlux(AuxQuantity1Phase):
  ...
  def compute(self, data, der):
    data[self.name] = data[self.u] * (data[self.arhoEA] + data[self.vf] * data[self.p] * data["A"])

    der[self.name]["aA1"] = data[self.u] * (der[self.vf]["aA1"] * data[self.p] + data[self.vf] * der[self.p]["aA1"]) * data["A"]
    ...
    der[self.name][self.arhoEA] = data[self.u] * (1 + data[self.vf] * der[self.p][self.arhoEA] * data["A"])
