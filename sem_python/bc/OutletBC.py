...
class OutletBC(OnePhaseBC):
  ...
  def applyWeakBC(self, U, r, J):
    ...
    if not self.strongly_enforce_energy:
      ...
      if (self.model_type == ModelType.TwoPhase):
        J[self.i_arhoEA, self.i_aA1] += (self.dvf_daA1 * self.u * (self.rho * self.E + self.p)
          + self.vf * (self.drho_daA1 * self.E + self.rho * self.dE_daA1)) * self.A * self.nx
      J[self.i_arhoEA, self.i_arhoA] += self.vf * (self.du_darhoA * (self.rho * self.E + self.p)
        + self.u * (self.drho_darhoA * self.E + self.rho * self.dE_darhoA)) * self.A * self.nx
      J[self.i_arhoEA, self.i_arhouA] += self.vf * (self.du_darhouA * (self.rho * self.E + self.p)
        + self.u * self.rho * self.dE_darhouA) * self.A * self.nx
