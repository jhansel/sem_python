...
def assertNonNegativeSoundSpeedArgSingle(arg, p, v):
    if arg < 0:
        error(
            "Sound speed: negative x in sqrt(x), where x = gamma * (p + p_inf) * v:\n" + "p = " +
            str(p) + "\nv = " + str(v))
...
class StiffenedGasEoS(EoS):
    ...
    def rho(self, p, T):
        ...
        drho_dT = -(p + self.p_inf) * ((self.gamma - 1) * self.cv * T)**-2 * (
            self.gamma - 1) * self.cv
    ...
    def rho_from_p_s(self, p, s):
        ...
        daux_ds = 1.0 / self.cv
        daux_dp = 1.0 / (p + self.p_inf)**(self.gamma - 1.0) * (self.gamma - 1.0) * (
            p + self.p_inf)**(self.gamma - 2.0)
    ...
    def s_from_h_p(self, h, p):
        aux = (p + self.p_inf) * ((h - self.q) /
                                  (self.gamma * self.cv))**(-self.gamma / (self.gamma - 1))
        daux_dh = (p + self.p_inf) * ((h - self.q) / (self.gamma * self.cv))**(-self.gamma/(self.gamma - 1) - 1) \
          * (-self.gamma/(self.gamma - 1)) / (self.gamma * self.cv)
        daux_dp = ((h - self.q) / (self.gamma * self.cv))**(-self.gamma / (self.gamma - 1))

        s = self.q_prime - (self.gamma - 1) * self.cv * log(aux)
