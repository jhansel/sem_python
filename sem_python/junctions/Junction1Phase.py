...
class Junction1Phase(Junction):
    ...
    def computeFlowQuantities(self, U):
        for i in range(self.n_meshes):
            ...
            self.vf[i], self.dvf_daA1[i] = computeVolumeFraction(
                aA1, self.A[i], self.phase, self.model_type)

            self.rho[i], drho_dvf, self.drho_darhoA[i], _ = computeDensity(
                self.vf[i], arhoA, self.A[i])
            ...
            self.E[i], self.dE_darhoA[i], self.dE_darhoEA[i] = computeSpecificTotalEnergy(
                arhoA, arhoEA)
            ...
            self.h[i], dh_de, dh_dp, dh_drho = computeSpecificEnthalpy(
                self.e[i], self.p[i], self.rho[i])
            self.dh_daA1[i] = dh_dp * self.dp_daA1[i]
            self.dh_darhoA[
                i] = dh_de * self.de_darhoA[i] + dh_dp * self.dp_darhoA[i] + dh_drho * self.drho_darhoA[i]
    ...
    def computeFluxes(self, U):
        for i in range(self.n_meshes):
            ...
            self.df_momentum_darhoA[i] = (
                arhouA * self.du_darhoA[i] + vf * self.dp_darhoA[i] * A) * nx
            self.df_momentum_darhouA[i] = (
                arhouA * self.du_darhouA[i] + u + vf * self.dp_darhouA[i] * A) * nx
            ...
            self.df_energy_darhoA[i] = (
                (arhoEA + vf * p * A) * self.du_darhoA[i] + vf * self.dp_darhoA[i] * A * u) * nx
