...
class NewerCompressibleJunction(Junction1Phase):
    ...
    def __init__(self, params):
        ...
        if params.has("loss_coefficients"):
            ...
            if len(self.loss_coefficients) != self.n_meshes:
                error(
                    "The list parameters 'loss_coefficients' and 'mesh_names' must have the same size."
                )
        else:
    ...
    def computeJunctionStagnationEnthalpy(self):
        ...
        for i in range(self.n_meshes):
            if self.u[i] * self.nx[i] > 0:  # mesh serves as inlet
                ...
                dnum_darhoEA[i] = self.rho[i] * self.u[i] * self.dh0_darhoEA[i] * self.A[
                    i] * self.nx[i]
                dden_darhoA[i] = (
                    self.drho_darhoA[i] * self.u[i] + self.rho[i] * self.du_darhoA[i]
                ) * self.A[i] * self.nx[i]
    ...
    def addJunctionOutletFlux(self, i, U, r, J):
        ...
        J[i_momentum][i_mass] += vf * (
            drho_darhoA * u**2 + rho * 2 * u * self.du_darhoA[i] + dp_darhoA) * A * nx
        J[i_momentum][i_momentum] += vf * (
            drho_darhouA * u**2 + rho * 2 * u * self.du_darhouA[i] + dp_darhouA) * A * nx
