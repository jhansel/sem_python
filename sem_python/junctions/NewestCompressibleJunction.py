...
class NewestCompressibleJunction(Junction1Phase):
    ...
    def addJunctionInletFlux(self, i, U, r, J):
        ...
        self.df_energy_daA1[i] = (
            dvf_daA1 * rho * H + vf * drho_daA1 * H + vf * rho * dH_daA1) * u * A * nx
        self.df_energy_darhoA[i] = vf * (
            drho_darhoA * u * H + rho * du_darhoA * H + rho * u * dH_darhoA) * A * nx
        ...
        self.df_entropy_daA1[i] = (
            dvf_daA1 * rho * s + vf * drho_daA1 * s + vf * rho * ds_daA1) * u * A * nx
        self.df_entropy_darhoA[i] = vf * (
            drho_darhoA * u * s + rho * du_darhoA * s + rho * u * ds_darhoA) * A * nx
    ...
    def addJunctionOutletFlux(self, i, U, r, J):
        ...
        df_momentum_darhoA = vf * (
            drho_darhoA * u**2 + rho * 2 * u * du_darhoA + dp_darhoA) * A * nx
        df_momentum_darhouA = vf * (
            drho_darhouA * u**2 + rho * 2 * u * du_darhouA + dp_darhouA) * A * nx
