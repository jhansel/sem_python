...
class DissipationVariableGradient(Kernel1Phase):
    ...
    def computeJacobian(self, data, der, var_index, i, j):
        ...
        if var_index == self.aA1_index:
            return (
                der[self.visccoef]["aA1"] * data["phi"][j] * data[self.grad_var] +
                data[self.visccoef] * dgrad_vari_dvarj) * data["grad_phi"][i] * data["JxW"]
        elif var_index == self.arhoA_index:
            return (
                der[self.visccoef][self.arhoA] * data["phi"][j] * data[self.grad_var] +
                data[self.visccoef] * dgrad_vari_dvarj) * data["grad_phi"][i] * data["JxW"]
        elif var_index == self.arhouA_index:
            return (
                der[self.visccoef][self.arhouA] * data["phi"][j] * data[self.grad_var] +
                data[self.visccoef] * dgrad_vari_dvarj) * data["grad_phi"][i] * data["JxW"]
        elif var_index == self.arhoEA_index:
            return (
                der[self.visccoef][self.arhoEA] * data["phi"][j] * data[self.grad_var] +
                data[self.visccoef] * dgrad_vari_dvarj) * data["grad_phi"][i] * data["JxW"]
        else:
            return self.zero
