...
class EnergyHeatTransfer(Kernel1Phase):
    ...
    def computeResidual(self, data, i):
        return -data["htc_wall"] * (
            data["T_wall"] - data[self.T]) * data["P_heat"] * data["phi"][i] * data["JxW"]

    def computeJacobian(self, data, der, var_index, i, j):
        if var_index == self.aA1_index:
            return data["htc_wall"] * der[self.T]["aA1"] * data["P_heat"] * data["phi"][j] * data[
                "phi"][i] * data["JxW"]
        elif var_index == self.arhoA_index:
            return data["htc_wall"] * der[self.T][self.arhoA] * data["P_heat"] * data["phi"][
                j] * data["phi"][i] * data["JxW"]
        elif var_index == self.arhouA_index:
            return data["htc_wall"] * der[self.T][self.arhouA] * data["P_heat"] * data["phi"][
                j] * data["phi"][i] * data["JxW"]
        elif var_index == self.arhoEA_index:
            return data["htc_wall"] * der[self.T][self.arhoEA] * data["P_heat"] * data["phi"][
                j] * data["phi"][i] * data["JxW"]
        else:
            return self.zero
