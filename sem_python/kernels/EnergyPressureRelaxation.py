...
class EnergyPressureRelaxation(Kernel2Phase):
  ...
  def computeResidual(self, data, i):
    return self.sign * data["pI_bar"] * data["p_relax"] * (data["p1"] - data["p2"]) * data["A"] * data["phi"][i] * data["JxW"]

  def computeJacobian(self, data, der, var_index, i, j):
    if var_index == self.aA1_index:
      return self.sign * (der["pI_bar"]["aA1"] * data["p_relax"] * (data["p1"] - data["p2"]) + \
        data["pI_bar"] * der["p_relax"]["aA1"] * (data["p1"] - data["p2"]) + \
        data["pI_bar"] * data["p_relax"] * (der["p1"]["aA1"] - der["p2"]["aA1"])) * \
        data["A"] * data["phi"][j] * data["phi"][i] * data["JxW"]
