...
class NonlinearSolverParameters(Parameters):
    ...
    def __init__(self, factory):
        ...
        self.registerFloatParameter(
            "relative_tolerance", "Relative tolerance for nonlinear solve", 1e-6)
        self.registerIntParameter("max_iterations", "Maximum number of nonlinear iterations", 10)

        self.registerBoolParameter(
            "print_variable_residual_norms",
            "Option to print the individual variable residual norms", False)
        self.registerBoolParameter("print_residual", "Option to print the residual vector", False)
        self.registerBoolParameter(
            "print_condition_number", "Option to print Jacobian condition number", False)
        self.registerBoolParameter("debug_jacobian", "Option to debug the Jacobian", False)
        self.registerBoolParameter(
            "visualize_sparsity", "Option to visualize the Jacobian sparsity pattern", False)
        self.registerFloatParameter(
            "finite_difference_eps", "Parameter for the FD debug Jacobian", 1e-4)
        self.registerBoolParameter(
            "print_jacobian_inverse", "Option to print the inverse of the Jacobian", False)
        self.registerBoolParameter("verbose", "Option to print out iterations", True)
        self.registerBoolParameter(
            "print_svd", "Option to print singular value decomposition", False)
        self.registerBoolParameter("print_eigenvalues", "Option to print eigenvalues", False)
        self.registerBoolParameter(
            "save_linear_system", "Option to save nonlinear residual and jacobian to CSV files",
            False)
...
class NonlinearSolver(object):
    ...
    def __init__(self, params):
        ...
        self.scaling[VariableName.ARhoA] = [
            params.get("scaling_arhoA1"),
            params.get("scaling_arhoA2")
        ]
        self.scaling[VariableName.ARhoUA] = [
            params.get("scaling_arhouA1"),
            params.get("scaling_arhouA2")
        ]
        self.scaling[VariableName.ARhoEA] = [
            params.get("scaling_arhoEA1"),
