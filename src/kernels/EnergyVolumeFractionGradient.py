...
class EnergyVolumeFractionGradient(Kernel2Phase):
  ...
  def computeResidual(self, data, i):
    return - self.sign * data["pI"] * data["uI"] * data["dvf1_dx"] * data["phi"][i] * data["JxW"]

  def computeJacobian(self, data, der, var_index, i, j):
    if var_index == self.vf1_index:
      return - self.sign * (der["pI"]["vf1"] * data["uI"] * data["dvf1_dx"] * data["phi"][j] \
      ...
      if var_index == self.arho1_index:
        ...
        return der["pI"]["arhoE1"] * data["uI"] * aux
        ...
        return der["pI"]["arhoE2"] * data["uI"] * aux
      else:
        return self.zero
