...
class VolumeFractionAdvection(Kernel2Phase):
  ...
  def computeJacobian(self, data, der, var_index, i, j):
    if var_index == self.aA1_index:
      return (der["uI"]["aA1"] * data["grad_vf1"] * data["phi"][j] + data["uI"] * der["grad_vf1"]["grad_aA1"] * data["grad_phi"][j]) * data["A"] * data["phi"][i] * data["JxW"]
    elif var_index == self.arhoA1_index:
      return der["uI"]["arhoA1"] * data["grad_vf1"] * data["A"] * data["phi"][j] * data["phi"][i] * data["JxW"]
    elif var_index == self.arhouA1_index:
      return der["uI"]["arhouA1"] * data["grad_vf1"] * data["A"] * data["phi"][j] * data["phi"][i] * data["JxW"]
    elif var_index == self.arhoEA1_index:
      return der["uI"]["arhoEA1"] * data["grad_vf1"] * data["A"] * data["phi"][j] * data["phi"][i] * data["JxW"]
    elif var_index == self.arhoA2_index:
      return der["uI"]["arhoA2"] * data["grad_vf1"] * data["A"] * data["phi"][j] * data["phi"][i] * data["JxW"]
    elif var_index == self.arhouA2_index:
      return der["uI"]["arhouA2"] * data["grad_vf1"] * data["A"] * data["phi"][j] * data["phi"][i] * data["JxW"]
    elif var_index == self.arhoEA2_index:
      return der["uI"]["arhoEA2"] * data["grad_vf1"] * data["A"] * data["phi"][j] * data["phi"][i] * data["JxW"]
    else:
      return self.zero
