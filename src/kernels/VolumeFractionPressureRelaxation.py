  ...
  def computeJacobian(self, data, der, var_index, i, j):
    if var_index == self.vf1_index:
      ...
      return -(data["p_relax"] * der["p1"]["arhoA1"] + \
        der["p_relax"]["arhoA1"] * (data["p1"] - data["p2"])) * data["phi"][j] * data["phi"][i] * data["JxW"]
    elif var_index == self.arhouA1_index:
      return -(data["p_relax"] * der["p1"]["arhouA1"] + \
        der["p_relax"]["arhouA1"] * (data["p1"] - data["p2"])) * data["phi"][j] * data["phi"][i] * data["JxW"]
    elif var_index == self.arhoEA1_index:\
      return -(data["p_relax"] * der["p1"]["arhoEA1"] + \
        der["p_relax"]["arhoEA1"] * (data["p1"] - data["p2"])) * data["phi"][j] * data["phi"][i] * data["JxW"]
    elif var_index == self.arhoA2_index:
      return -(-data["p_relax"] * der["p2"]["arhoA2"] + \
        der["p_relax"]["arhoA2"] * (data["p1"] - data["p2"])) * data["phi"][j] * data["phi"][i] * data["JxW"]
    elif var_index == self.arhouA2_index:
      return -(-data["p_relax"] * der["p2"]["arhouA2"] + \
        der["p_relax"]["arhouA2"] * (data["p1"] - data["p2"])) * data["phi"][j] * data["phi"][i] * data["JxW"]
    elif var_index == self.arhoEA2_index:
      return -(-data["p_relax"] * der["p2"]["arhoEA2"] + \
        der["p_relax"]["arhoEA2"] * (data["p1"] - data["p2"])) * data["phi"][j] * data["phi"][i] * data["JxW"]
    else:
      return self.zero
