...
class KernelDerivativesTester(object):
    ...
    def computeSolutionDependentData(self, U, data):
        ...
        data["grad_arhoA1"] = self.fe_values.computeLocalSolutionGradient(
            U, VariableName.ARhoA, 0, self.elem)
        data["grad_arhouA1"] = self.fe_values.computeLocalSolutionGradient(
            U, VariableName.ARhoUA, 0, self.elem)
        data["grad_arhoEA1"] = self.fe_values.computeLocalSolutionGradient(
            U, VariableName.ARhoEA, 0, self.elem)
        if self.model_type != ModelType.OnePhase:
            data["arhoA2"] = self.fe_values.computeLocalSolution(
                U, VariableName.ARhoA, 1, self.elem)
            data["arhouA2"] = self.fe_values.computeLocalSolution(
                U, VariableName.ARhoUA, 1, self.elem)
            data["arhoEA2"] = self.fe_values.computeLocalSolution(
                U, VariableName.ARhoEA, 1, self.elem)
