...
class DissipationAuxFluxDerivativesTester(unittest.TestCase):
    ...
    def test1Phase(self):
        aux = {
            "viscflux_arhouA1":
                ["arhoA1", "arhouA1", "arhoEA1", "grad_arhoA1", "grad_arhouA1", "grad_arhoEA1"]
        }
        kernel_params = {"var_enum": VariableName.ARhoUA, "flux_name": "viscflux_arhouA1"}
        rel_diffs = self.derivatives_tester.checkDerivatives(
            "DissipationAuxFlux", ModelType.OnePhase, 0, aux, kernel_params=kernel_params)
    ...
    def test2Phase(self):
        aux = {
            "viscflux_arhouA1":
                [
                    "aA1", "arhoA1", "arhouA1", "arhoEA1", "grad_aA1", "grad_arhoA1",
                    "grad_arhouA1", "grad_arhoEA1"
                ]
        }
        kernel_params = {"var_enum": VariableName.ARhoUA, "flux_name": "viscflux_arhouA1"}
        rel_diffs = self.derivatives_tester.checkDerivatives(
            "DissipationAuxFlux", ModelType.TwoPhase, 0, aux, kernel_params=kernel_params)
        for key in rel_diffs:
            self.assertLessEqual(rel_diffs[key], 1e-6)
