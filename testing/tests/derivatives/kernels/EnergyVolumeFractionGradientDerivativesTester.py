...
aux = {
    "uI": ["aA1", "arhoA1", "arhouA1", "arhoEA1", "arhoA2", "arhouA2", "arhoEA2"],
...
class EnergyVolumeFractionGradientDerivativesTester(unittest.TestCase):
    ...
    def testPhase1(self):
        rel_diffs = self.derivatives_tester.checkDerivatives(
            "EnergyVolumeFractionGradient", ModelType.TwoPhase, 0, aux, aux_gradients=["vf1"])
        for key in rel_diffs:
            self.assertLessEqual(rel_diffs[key], 1e-5)

    def testPhase2(self):
        rel_diffs = self.derivatives_tester.checkDerivatives(
            "EnergyVolumeFractionGradient", ModelType.TwoPhase, 1, aux, aux_gradients=["vf1"])
        for key in rel_diffs:
            self.assertLessEqual(rel_diffs[key], 1e-5)
